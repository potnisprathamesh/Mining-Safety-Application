...
public class Steering : MonoBehaviour {
	...
	public GameObject RMote,LMote;
	public RaycastHit objInfoR, objInfoL;
	...
	void Update () {
		...
		if(InputBroker.GetKeyDown(WiimoteName + ":Up")) {
			...
			fwd.y = 0;
			CommonVariables.mappedPosition.z += transform.forward.z * (direction.z/direction.z) * TravelSpeed * Time.deltaTime;
			CommonVariables.mappedPosition.x += transform.forward.x * (direction.x/direction.x) * TravelSpeed * Time.deltaTime;
		}
